...
function Navbar() {
  ...
  const [BarIcon, setBarIcon] = useState('fa-bars');
  const [NavLink, setNavLink] = useState('');
  const toggleBar = () =>{
    setBarShow(`${barShow === 'NavBarShow' ? 'NavBarHidden' : 'NavBarShow'}`);
    setBarIcon(`${BarIcon === 'fa-bars' ? 'fa-xmark' : 'fa-bars'}`);
                    ...
                    <div className='header-userData'>
                        <div>
                          <Link to='/Cart' className='header-userData-icon'>
                            <FaCartShopping className='position-relative cart-icon' />
                            <span className="position-absolute translate-middle text-light bg-danger rounded-circle">
                                {total_item}
                            </span>
                          </Link>
                        </div>
                        <div className='d-flex'>
                          {
                              userData ? 
                                  <button onClick={logout} className='mt-0 ms-4 authentication-btn'>
                                      <FaLockOpen className='me-1' />
                                      Logout
                                  </button> :
                                  <Link to='/Login' className='header-userData-icon ms-4'>
                            ...
                            <button onClick={toggleBar} className='header-bar'>
                                <i className={`fa-solid ${BarIcon}`}></i>
                            </button>
                        </div>
                    </div>
                </div>
